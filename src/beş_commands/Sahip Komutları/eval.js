...
module.exports = {
    ...
    aliases: [], execute: async (client, message, args, beş_embed) => {
        ...
        let code = args.join(" ");
        if (code.includes(client.token)) return message.reply({ content: "> **Bu Token itz.narutovonfrost Tarafından Koruma Altında ;)**" });
        try {
            var sonuç = eval_beş(await eval(code));
            if (sonuç.includes(client.token))
                return message.reply({ content: "> **Bu Token itz.narutovonfrost Tarafından Koruma Altında ;)**" });
        } catch (err) { }
    },
