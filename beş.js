...
client.ceza = async function (id, message, type, reason, durations, süre) {
  ...
  let embed = new EmbedBuilder().setColor("#2f3136").setAuthor({ name: message.member.displayName, iconURL: message.author.avatarURL({ dynamic: true, size: 2048 }) }).setFooter({ text: beş_config.footer ? beş_config.footer : `İtz.NarutoVonFrost Was Here`, iconURL: message.author.avatarURL({ dynamic: true, size: 2048 }) })
  switch (type) {
    case 'BAN':
